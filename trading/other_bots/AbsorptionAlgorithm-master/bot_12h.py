...
PAIRS = [
            'BTC_ETH', 'BTC_XRP', 'BTC_XEM', 'BTC_LTC', 'BTC_STR', 'BTC_BCN', 'BTC_DGB', 'BTC_ETC', 'BTC_SC', 'BTC_DOGE',
...
def is_dodge(candle):
    if (candle['high'] - candle['low']) / (abs(candle['close'] - candle['open']) + 0.0001) > COEF_ALL_CANDLE_MAX 
        and COEF_HIGH_LOW_MIN < (candle['high'] - max([candle['close'], candle['open']])) / (min([candle['close'], candle['open']]) - candle['low'] + 0.0001) < COEF_HIGH_LOW_MAX:
        return True
    else:
        return False
    ...
    for candle in candles_data:
        ...
        if len(candle_3na4) == 3:
            candle_12h = {'high': (
                ...
                'low': (min([float(candle_3na4[0]['low']), float(candle_3na4[1]['low']), float(candle_3na4[2]['low'])])),
                'volume': (sum([float(candle_3na4[0]['volume']), float(candle_3na4[1]['volume']), float(candle_3na4[2]['volume'])])),
